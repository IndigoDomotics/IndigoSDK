...
class Plugin(indigo.PluginBase):
    ...
    def shutdown(self):
        ...
        key_value_list = list()
        key_value_list.append({'key': 'serverTimeSeconds', 'value': 0})
        key_value_list.append({'key': 'serverSecondsEven', 'value': False})
        key_value_list.append({'key': 'serverDateTime', 'value': "--"})
    ...
    def runConcurrentThread(self):
        try:
            while True:
                ...
                key_value_list = list()
                key_value_list.append({'key': 'serverTimeSeconds', 'value': server_time_second})
                key_value_list.append({'key': 'serverSecondsEven', 'value': not bool(server_time_second % 2)})
                key_value_list.append({'key': 'serverDateTime', 'value': str(server_time)})
                self.server_time_dev.updateStatesOnServer(key_value_list)
    ...
    def addDevice(self, values_dict, type_id, dev_id):
        ...
        if "sourceDeviceMen" in values_dict:
            ...
            selected_devices_string = values_dict.get("memberDevices", "")
            self.logger.debug(f"adding device: {device_id} to {selected_devices_string}")
            ...
            if selected_devices_string == "":
                selected_devices_string = device_id
            # Otherwise append it to the end separated by a comma
            else:
                selected_devices_string += f",{device_id}"
            # Set the device string back to the hidden text field that contains the
            # list of device ids that are in the scene
            values_dict["memberDevices"] = selected_devices_string
            self.logger.debug("values_dict = " + str(values_dict))
            ...
            if "sourceDeviceMen" in values_dict:
                del values_dict["sourceDeviceMen"]
            # return the new dict
            return values_dict
    ...
    # device config UI.
    ####################
    def deleteDevices(self, values_dict, type_id, dev_id):
        ...
        if "memberDevices" in values_dict:
            ...
            devices_in_scene = values_dict.get("memberDevices", "").split(",")
            # Get the devices they've selected in the list that they want
            # to remove
            selected_devices = values_dict.get("memberDeviceList", [])
    ...
    ####################
    def sourceDevices(self, filter_str="", values_dict=None, type_id="", target_id=0):
        self.logger.debug(f"sourceDevices called with filter: {filter_str}  type_id: {type_id}  target_id: {target_id}")
        return_list = list()
        ...
        device_list = values_dict.get("memberDevices", "").split(",")
    ...
    # that values_dict is read-only so any changes you make to it will be discarded.
    ####################
    def memberDevices(self, filter_str="", values_dict=None, type_id="", target_id=0):
        self.logger.debug(f"memberDevices called with filter: {filter_str}  type_id: {type_id}  target_id: {target_id}")
        return_list = list()
        # values_dict may be empty or None if it's a brand new device
        if values_dict and "memberDevices" in values_dict:
            # Get the list of devices
            deviceListString = values_dict["memberDevices"]
            self.logger.debug("memberDeviceString: " + deviceListString)
            device_list = deviceListString.split(",")
    ...
    def validateDeviceConfigUi(self, values_dict, type_id, dev_id):
        ...
        # care about those.
        self.logger.debug(f"validateDeviceConfigUi: type_id: {type_id}  devId: {dev_id}")
        if type_id == "scene":
            if "memberDeviceList" in values_dict:
                values_dict["memberDeviceList"] = ""
            if "sourceDeviceMen" in values_dict:
                values_dict["sourceDeviceMen"] = ""
    ...
    def reset_hardware(self, action):
        self.logger.debug("reset_hardware action called:\n" + str(action))

    def update_hardware_firmware(self, action):
        self.logger.debug("update_hardware_firmware action called:\n" + str(action))
